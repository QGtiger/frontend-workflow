...
// ============ 5. 状态判定函数 ============
function getExpressionState(error: Error | null): ExpressionState {
...
function buildDecorations(state: EditorState): DecorationSet {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    const { error } = evaluateExpression(exprContent);
...
// ============ 8. 导出完整的扩展 ============
export const highlightExpressions = [expressionTheme, highlightField];
